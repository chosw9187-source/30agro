...
import { GOAL_CYCLE_STATUS_LABEL, type GoalCycleStatus } from "@/lib/goals";
import { CycleSelect } from "@/app/platform/evaluation2/cycle-select";
import { ActionForm } from "@/components/action-form";
import { includeAllEvalTargets, setEvalTarget } from "./actions";
...
export default async function EvalTargetsPage({
  ...
  const people: Person[] = users.map((u) => {
    const row = targetByUser.get(u.id);
    return {
      id: u.id,
      name: u.name,
      ...
      // 행이 없으면 대상이다 - 새로 들어온 사람이 자동으로 명단에 잡히도록.
  ...
  const includedCount = people.filter((p) => p.included).length;
  const excludedCount = people.length - includedCount;
  ...
  const withoutGoal = people.filter(
    (p) => p.included && needsOwnGoal(p) && p.goalCount === 0
  ).length;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!cycle ? (
        ...
        <>
          <section className={`${CARD_CLASS} flex flex-wrap items-center gap-x-6 gap-y-2 px-4 py-3`}>
            <p className="text-sm text-slate-500">
              명단은 조직도와 같은 곳에서 옵니다 — 재직 중인 인원이 자동으로 대상에 잡히고, 여기서
              뺀 사람만 집계에서 빠집니다.
            </p>
            <div className="ml-auto flex items-center gap-5 whitespace-nowrap">
              ...
              <span className="text-sm text-slate-500">
                제외 <b className="tabular-nums">{excludedCount}</b>명
              </span>
              <span className="text-sm text-slate-500">
                개인목표 없음 <b className="tabular-nums">{withoutGoal}</b>명
              </span>
              {excludedCount > 0 && (
                <ActionForm action={includeAllEvalTargets} successMessage="정상 반영되었습니다.">
                ...
                <ul className="divide-y divide-slate-100">
                  {g.people.map((p) => (
                    <li key={p.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-2">
                      ...
                      {p.included && needsOwnGoal(p) && p.goalCount === 0 && (
